...
# 			loss = calculate_loss(visual_data_input, audio_data_gt, audio_reconstruct,  mu, logvar, syn_label)
...
# 			loss = calculate_loss(audio_data_gt, visual_data_input, visual_reconstruct,  mu, logvar, syn_label)
# 			distance_loss = loss.item()
...
if __name__ == '__main__':
	...
	vae_audio.load_state_dict(torch.load('vae_audio_baseline4_mergeloss.pkl'))
	vae_video.load_state_dict(torch.load('vae_video_baseline4_mergeloss.pkl'))
	# vae_audio_a2v = VAE(latent_dim, audio_encode, audio_decode)
	# vae_video_a2v = VAE(latent_dim, video_encode, video_decode)
	# vae_audio_a2v.cuda()
	# vae_video_a2v.cuda()

	# vae_audio_v2a = VAE(latent_dim, audio_encode, audio_decode)
	# vae_video_v2a = VAE(latent_dim, video_encode, video_decode)
	# vae_audio_v2a.cuda()
	# vae_video_v2a.cuda()

	# vae_audio_a2v.load_state_dict(torch.load('vae_audio_baseline2.pkl'))
	# vae_video_a2v.load_state_dict(torch.load('vae_video_baseline2.pkl'))

	# vae_audio_v2a.load_state_dict(torch.load('vae_audio_baseline2.pkl'))
	# vae_video_v2a.load_state_dict(torch.load('vae_video_baseline2.pkl'))
